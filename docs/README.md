...
介護業界（看護や保育含）の求職者向けサービスを展開しているが、近年 CVR が悪く、売上が上がらない状況が続いていた。運営側と協力し、CVR 改善や SEO 改善施策に取り組む。
...
- **その他：**
    - ソースのリファクタリング
- **実績：**
  - すでに別会社が作ったサービスを当社が業務委託として引き受けることになったサービス。近年売上が落ち込んでいることから、売上アップに貢献するため、CVR の改善や SEO 対策の開発業務を進めることで、マーケティングの視点を持った開発業務・GA や SearchConsole 等使った CVR 数値分析を経験できた
  - サービスがスタートしたのが 2018 年で、フレームワークのバージョンも古くなっており、バージョンアップ対応等の貴重な経験をできた
...
- **実績：**
    ...
    - 特に苦労したのが、GraphQL を用いた GCP の CloudStorage への画像アップロード機能。まだまだ GraphQL の技術は参考文献が少なく、Laravel Lighthouse(Laravel で GraphQL を利用する際のライブラリ）公式ドキュメントを見ながら、あまり知見のない機能を自力で開発できた。テストコードもダミー画像を使って実装した
...
- **実績**
    ...
    - アメリカ大手の半導体メーカーに対し、これまで再生が難しく、汚染したら新規装置を購入するという運用状態に手を入れ、完全新規洗浄処方技術を開発し、アメリカで運用をスタートさせた
    - 上記開発によって大幅なコスト削減につなげた
...
- **実績：**
    ...
    - MA システム関係に関しては、MA アナリティクス画面は、複数データを紐づけて表示させているが、そのデータ作成は生 SQL で動いていた
    - その画面表示させるための SQL の実装をしていたが、特に複雑なデータやデータ量の多いお客さんのアナリティクス画面の表示が遅いといった要望が多かった
...
- **実績**
    ...
    - また、各自の技術に依存し、トラブルの多かった生産ラインの手順化（機械の設置、生産時の温度管理など）を進め、誰でも生産ラインを担当できるような運用を構築した
...
- **実績：**
    ...
    - 0→1 のサービス構築をほぼ 1 人で実装する経験をさせていただいた
...
プログラミングスクールで案件開発の経験を積んでもらうための教材作成。システムの修正、機能追加案件を想定して教材を作るため、修正もととなるシステムが必要になった。一般的な CRM（顧客関係管理）に近いシステムを 0 から作り、生徒様向けに模擬案件として、実際の業務と同じように開発・修正できるものにした。
